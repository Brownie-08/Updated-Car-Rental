...
from .payment_services import StripePaymentService, PaystackPaymentService, PayLaterService
    ...
    try:
        if payment_method == 'stripe':
            ...
            callback_url = request.build_absolute_uri(
                reverse('system:paystack_callback', args=[order_id])
            )
            
            result = PaystackPaymentService.initialize_transaction(order, callback_url)
            
            if result['success']:
                # Redirect to Paystack payment page with proper currency info
...
def paystack_callback(request, order_id):
    ...
    if not reference:
        ...
        return redirect('system:payment_cancelled', order_id=order_id)
    ...
    try:
        ...
        if result['success']:
            ...
            messages.success(request, 
                f'🎉 Payment successful! Your booking #{order.order_number} has been confirmed.')
            return redirect('system:payment_success', order_id=order_id)
        else:
            messages.error(request, f"Payment verification failed: {result['error']}")
            return redirect('system:payment_cancelled', order_id=order_id)
            
    except Exception as e:
        logger.error(f"Error in Paystack callback: {str(e)}")
...
def paystack_webhook(request):
    ...
    result = PaystackPaymentService.handle_webhook(payload, signature)
    
    if result['success']:
        return HttpResponse(status=200)
    else:
        logger.error(f"Paystack webhook processing failed: {result.get('error', 'Unknown error')}")
