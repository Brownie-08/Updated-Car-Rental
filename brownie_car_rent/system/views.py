...
def order_created(request, car_id=None):
    ...
    if form.is_valid():
        # Get selected payment method first
        payment_method = form.cleaned_data.get('payment_method', 'pay_later')
        
        # For immediate payment methods (Stripe, Paystack), don't create order yet
        # Order will be created after successful payment verification
        if payment_method in ['stripe', 'paystack']:
            ...
            order_data = {
                ...
                'cell_no': form.cleaned_data.get('cell_no'),
            ...
            if selected_car:
                order_data['daily_rate'] = selected_car.cost_par_day
            else:
                car_name = form.cleaned_data.get('car_name')
                if car_name:
                    try:
                        car = Car.objects.get(car_name=car_name)
                        order_data['daily_rate'] = car.cost_par_day
                    except Car.DoesNotExist:
                        order_data['daily_rate'] = 0
            
            # Create Order instance but don't save to DB yet
            instance = Order(**order_data)
            # Calculate total amount for payment processing
            instance.calculate_billing()
            
        else:
            # For pay_later, create order immediately
            instance = form.save(commit=False)
        ...
        if payment_method == 'stripe':
            # Save the order now that we're processing payment
            instance.save()
            ...
            instance.status = 'pending_payment'  # Don't confirm until payment is verified
            instance.save()
            
            # Redirect to Paystack payment processing
            from .payment_services import PaystackPaymentService
            
            callback_url = request.build_absolute_uri(
                reverse('system:paystack_callback', args=[instance.id])
            )
            
            result = PaystackPaymentService.initialize_transaction(instance, callback_url)
            if result['success']:
                # Redirect to Paystack checkout
                return render(request, 'system/payment/paystack_checkout.html', {
                    'order': instance,
                    'transaction': result['transaction'],
