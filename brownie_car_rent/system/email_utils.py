...
def send_payment_confirmation_email(order, transaction):
    """
    Send payment confirmation email to customer after successful payment.
    ...
    try:
        ...
        # Email subject
        subject = f"💳 Payment Confirmed - Order #{order.order_number} - Brownie Car Rental"
        ...
        Great news! Your payment has been successfully processed for your car rental booking.
        ...
        Payment Method: {transaction.payment_method.display_name}
        Payment Status: {transaction.get_status_display()}
        Transaction Date: {transaction.created_at.strftime('%B %d, %Y at %I:%M %p')}
        ...
        • Our team will contact you within 24 hours for pickup arrangements
        • Please bring valid driver's license and the credit card used for payment
        ...
        logger.info(f"Payment confirmation email sent successfully for order {order.order_number} to {customer_email}")
        return True
...
def send_cancellation_notification_email(order_details):
    """
    ...
    try:
        ...
        subject = f"❌ Booking Cancelled #{order_details.get('order_number', 'N/A')} - Brownie Car Rental"
...
This email confirms that your car rental booking has been cancelled.

CANCELLED BOOKING DETAILS:
========================
Order Number: #{order_details.get('order_number', 'N/A')}
Vehicle: {order_details.get('car_name', 'N/A')}
Rental Period: {order_details.get('pick_up_date', 'N/A')} - {order_details.get('drop_off_date', 'N/A')}
Total Amount: ${order_details.get('total_amount', '0.00')}

If this cancellation was made in error or if you have any questions, please contact our support team immediately.
...
Brownie Car Rental Team
        ...
        logger.info(f"Cancellation notification sent to {customer_email} for order {order_details.get('order_number')}")
        return True
...
def send_admin_cancellation_notification_email(order_details):
    """
    ...
    """
    try:
        subject = f"🔔 Order Cancelled #{order_details.get('order_number', 'N/A')} - Admin Alert"
        
        text_content = f"""
Order Cancellation Alert

An order has been cancelled in the system.
...
Please review the cancellation and follow up if necessary.
...
Admin Dashboard: http://localhost:8000/admin
        ...
        logger.info(f"Admin cancellation notification sent for order {order_details.get('order_number')}")
        return True
